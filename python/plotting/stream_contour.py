...
Options:
    --fig_name=<fig_name>               Name of figure output directory & base name of saved figures [default: stream_uw]
...
field_keys = ['u','b_mag', 'phi', 'Bx','Bz', 'w']
bases_keys  = ['x', 'z']
    ...
    if not plotter.idle:
        while plotter.files_remain(bases_keys, field_keys):
            ...
            for i, num in enumerate(write_num):
                ...
                p = ax1.pcolormesh(xx, zz, phi, cmap='viridis')
                plt.colorbar(p, cax1, orientation='horizontal')
                f=ax1.quiver(xx[skip],zz[skip],u[skip],w[skip], color='white',  units='xy', scale = 5)
                ...
                #h=ax1.quiver(xx[skip],zz[skip],Bx[skip],Bz[skip], color='white',  units='xy', scale = 150)
